...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*mtcutil.SatoshiPerBitcoin > mtcutil.MaxSatoshi {
			return nil, &btcjson.RPCError{
		...
		addr, err := mtcutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &btcjson.RPCError{
		...
		satoshi, err := mtcutil.NewAmount(amount)
		if err != nil {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = mtcutil.Amount(v.Value).ToBTC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	txReply := &btcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(mtcutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(mtcutil.NewTx(mtx))),
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := mtcutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := mtcutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         mtcutil.Amount(value).ToBTC(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     mtcutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr mtcutil.Address, numToSkip, numRequested uint32) ([]*mtcutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := mtcutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	wif, err := mtcutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case mtcutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case mtcutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := mtcutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
	...
	default:
		// Handle the case when a new Address is supported by mtcutil, but none
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, ok := addr.(*mtcutil.AddressPubKeyHash); !ok {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
	...
	address, err := mtcutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *mtcutil.Block, flags blockchain.BehaviorFlags) (bool, error)
