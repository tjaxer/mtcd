...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: mtcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := mtcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*mtcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, mtcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount mtcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee mtcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee mtcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee mtcutil.Amount) *wire.MsgTx {
