...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *mtcutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *mtcutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *mtcutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *mtcutil.Block,
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *mtcutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *mtcutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*mtcutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*mtcutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *mtcutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *mtcutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *mtcutil.Block, []SpentTxOut) error
}
