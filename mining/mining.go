...
const (
	...
	MinHighPriority = mtcutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr mtcutil.Address) (*mtcutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *mtcutil.Tx, height int32) error {
...
func logSkippedDeps(tx *mtcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress mtcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*mtcutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := mtcutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := mtcutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A mtcutil.Block should use saved in the state to avoid
