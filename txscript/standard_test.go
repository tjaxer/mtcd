...
// newAddressPubKey returns a new mtcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) mtcutil.Address {
	addr, err := mtcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new mtcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) mtcutil.Address {
	addr, err := mtcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new mtcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) mtcutil.Address {
	addr, err := mtcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the mtcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the mtcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := mtcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := mtcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := mtcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := mtcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := mtcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*mtcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*mtcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*mtcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*mtcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*mtcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
