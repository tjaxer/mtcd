...
import (
	...
	"github.com/tjaxer/mtcd/rpcclient"
...
type memWallet struct {
	...
	addrs map[uint32]mtcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]mtcutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*mtcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          mtcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (mtcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []mtcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt mtcutil.Amount,
	feeRate mtcutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := mtcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate mtcutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate mtcutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt mtcutil.Amount
	for _, output := range outputs {
		outputAmt += mtcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() mtcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance mtcutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (mtcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := mtcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
