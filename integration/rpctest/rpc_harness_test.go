...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt mtcutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(mtcutil.Amount(5 * mtcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(mtcutil.Amount(500 * mtcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(mtcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*mtcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, mtcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := mtcutil.Amount(250 * mtcutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = mtcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := mtcutil.Amount(50 * mtcutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := mtcutil.Amount(numMatureOutputs * 50 * mtcutil.SatoshiPerBitcoin)
