...
const (
	...
	DefaultMinRelayTxFee = mtcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee mtcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > mtcutil.MaxSatoshi {
		minFee = mtcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *mtcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee mtcutil.Amount) bool {
...
func checkTransactionStandard(tx *mtcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee mtcutil.Amount,
...
func GetTxVirtualSize(tx *mtcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
